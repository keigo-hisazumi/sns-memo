...
import { MemosProvider, useMemos } from './context/MemosContext'
...
function AppContent() {
  ...
  const { allMemos, filteredMemos, searchQuery, setSearchQuery, addMemo } = useMemos()
  const [currentPage, setCurrentPage] = useState('home')

  if (authLoading) {
    return (
      <div style={{ position: 'fixed', inset: 0, display: 'flex', justifyContent: 'center', alignItems: 'center', backgroundColor: 'var(--bg-primary)' }}>
  ...
  return (
    <>
      <header className="app-header">
        ...
        <span className="app-title">SNS Memo</span>
        <button className="logout-button" onClick={logout} title="ログアウト">
